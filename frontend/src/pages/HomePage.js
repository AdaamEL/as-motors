...
const HomePage = () => {
  ...
  const handleMobileScroll = (event) => {
    const viewportHeight = event.currentTarget.clientHeight || window.visualViewport?.height || window.innerHeight || 1;
  ...
  return (
    <div className="relative w-full max-w-full h-[100dvh] overflow-x-hidden overflow-y-hidden">
      ...
      <div
        className="md:hidden absolute left-0 top-0 w-full h-[100dvh] z-10 overflow-x-hidden overflow-y-auto no-scrollbar home-mobile-snap"
        onScroll={handleMobileScroll}
        aria-hidden="true"
      >
        {slides.map((slide) => (
          <section key={slide.index} className="h-[100dvh] w-full home-mobile-snap-section" />
        ))}
      </div>

      {/* Mobile overlay: cinematic title + index */}
      <div className="md:hidden fixed left-0 top-0 w-full h-[100dvh] z-20 pointer-events-none overflow-hidden">
