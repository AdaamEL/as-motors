...
const CookieBanner = ({ discrete = false }) => {
  ...
  // Discrete version for homepage
  if (discrete) {
    return (
      <div className="fixed bottom-6 right-6 z-40 max-w-sm">
        <div className="p-3 rounded-lg bg-black/80 backdrop-blur-md border border-white/10 shadow-lg">
          <div className="flex items-start gap-3">
            <div className="flex-1 min-w-0">
              <p className="text-xs text-white/80">
                Nous utilisons des cookies essentiels.{" "}
                <Link to="/legal/cookies" className="text-gold hover:text-gold/80 underline">
                  En savoir plus
                </Link>
              </p>
            </div>
            <div className="flex gap-2 ml-2 flex-shrink-0">
              <button
                ...
                className="px-3 py-1 rounded text-xs font-semibold text-white hover:text-gold transition-colors"
              >
                OK
              </button>
              <button
                onClick={() => saveConsent("essential", { essential: true, analytics: false })}
                className="px-3 py-1 rounded text-xs font-semibold text-white/60 hover:text-white transition-colors"
              >
                Refuser
              </button>
            </div>
